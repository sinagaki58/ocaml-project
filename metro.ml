...
let global_ekikan_list = [ 
  ...
  {kiten="代々木公園"; shuten="����治神宮前"; keiyu="千代��������������������������������������������������線"; kyori=1.2; jikan=2}; 
  ...
  {kiten="南阿佐ヶ���"; shuten="荻窪"; keiyu="丸ノ内線"; kyori=1.5; jikan=2}; 
  {kiten="中野坂上"; shuten="中野新橋"; keiyu="丸ノ内線"; kyori=1.3; jikan=2}; 
  {kiten="中野新橋"; shuten="������������������"; keiyu="丸ノ内線"; kyori=0.6; jikan=1}; 
  {kiten="���������士見町"; shuten="方南町"; keiyu="丸ノ内線"; kyori=1.3; jikan=2}; 
...
let rec romaji_to_kanji name list = match list with
    [] -> ""
  | ekimei_t::rest -> if ekimei_t.romaji = name then ekimei_t.kanji else romaji_to_kanji name rest

let rec get_ekikan_kyori name1 name2 ls = match ls with
    [] -> infinity
  | ekikan_t::rest -> if (ekikan_t.kiten = name1 && ekikan_t.shuten = name2) || (ekikan_t.kiten = name2 && ekikan_t.shuten = name1) 
    then ekikan_t.kyori
...
let kyori_wo_hyoji roma1 roma2 = 
  ...
  if kanji1 = "" then roma1 ^ " という駅は存在しません"
    ...
    if kanji2 = "" then roma2 ^ " という駅は存在しません"
      ...
      if kyori != infinity
      then  kanji1 ^ "から" ^ kanji2 ^ "までは " ^ string_of_float kyori ^ " キロです"
      else kanji1 ^ "と" ^  kanji2 ^ "はつながっていません"

let make_initial_eki_list ls shiten = List.map (fun ekimei -> 
    if ekimei.kanji = shiten 
    then {namae= ekimei.kanji; saitan_kyori= 0.; temae_list= ekimei.kanji::[]}
    else {namae= ekimei.kanji; saitan_kyori= infinity; temae_list= []}
  ) ls

let rec ekimei_insert ls ekimei0 = match ls with
    [] -> ekimei0::[]
  | ekimei_t as first::rest -> 
    if first.kana > ekimei0.kana 
    then ekimei0::ls 
    else if first.kana = ekimei0.kana 
    then ekimei_insert rest ekimei0 
    else first::ekimei_insert rest ekimei0

let rec seiretsu ls = match ls with
    [] -> []
  | ekimei_t as first::rest -> ekimei_insert (seiretsu rest) first
